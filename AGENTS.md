...
`docs/NOVELTY.md`); do not ship any claim that it is. The portable value is the
...
1. Set up per `docs/REPRODUCE.md`, run at least the 600-step comparison once, and
...
3. Nothing about the headline claim changes without an explicit note in
   `docs/RESULTS.md` and `docs/PROVENANCE.md`.
...
3. **Recall probe:** Status: **done (v3)** — full numbers in
   `docs/PROBE_RESULTS.md` and raw logs in `results/recall_probe/probe3_*_1500_log.json`.
   ...
   `docs/RESULTS.md`). Next worthwhile step is longer context (train.py `SEQ`)
   or a bigger model to climb beyond 26% and confirm the ordering at scale.
